...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  with tf.name_scope(layer_name):
    fc1_outsize = int(BOTTLENECK_TENSOR_SIZE**1.5)
    layer_fc1  = new_fc_layer(
      input_layer=bottleneck_input,    # The previous layer.
      num_inputs=BOTTLENECK_TENSOR_SIZE,     # Num. inputs from prev. layer.
      num_outputs=fc1_outsize,    # Num. outputs.
      layer_name='fc1',
      use_relu=True,
    )
    layer_fc2  = new_fc_layer(
      input_layer=layer_fc1,    # The previous layer.
      num_inputs=fc1_outsize,     # Num. inputs from prev. layer.
      num_outputs=class_count,    # Num. outputs.
      layer_name='fc2',
      use_relu=False,
    )
    tf.summary.histogram('pre_activations', layer_fc2)

  final_tensor = tf.nn.softmax(layer_fc2, name=final_tensor_name)
  tf.summary.histogram('activations', final_tensor)

  with tf.name_scope('cross_entropy'):
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
      layer_fc2, ground_truth_input)
...
def new_fc_layer(input_layer,    # The previous layer.
    ...
    weights_name = 'weights_'+layer_name
    with tf.name_scope(weights_name):
      weights = new_weights(shape=[num_inputs, num_outputs], name=weights_name)
      variable_summaries(weights)
    biases_name = 'biases_'+layer_name
    with tf.name_scope(biases_name):
      biases = new_biases(length=num_outputs, name=biases_name)
      variable_summaries(biases)
    # Calculate the layer as the matrix multiplication of
    # the input and weights, and then add the bias-values.
    with tf.name_scope('Wx_plus_b_'+layer_name):
